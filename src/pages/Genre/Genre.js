    ...
    useEffect(() => {
        const fetchAPIBrand = async (page) => {
            try {
                ...
                if (genreName === 'Unisex') {
                    setGenre(2);
                } else if (genreName === 'Nam') {
                    setGenre(1);
                } else if (genreName === 'Nữ') {
                    setGenre(0);
                }
                const result = await productsServices.productforgenre(
                    genre,
                    brandName,
        ...
        // fetchAPIProduct();
    }, [genre, genreName, brandName, selectedPrices, priorityCheckboxState, fetchAPI]); // Dependency array trống đại diện cho việc useEffect chỉ chạy một lần như componentDidMount
